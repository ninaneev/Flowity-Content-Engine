...
class Post(Base):
    ...
    channel: Mapped[str] = mapped_column(
        String(20), default="linkedin", comment="linkedin | x | newsletter"
    )
    tone: Mapped[str | None] = mapped_column(
        String(50), comment="estratégico | educativo | inspiracional | direto"
    )
    objective: Mapped[str | None] = mapped_column(String(100))
    format: Mapped[str | None] = mapped_column(
        String(50), comment="lista | narrativa | pergunta | dado"
    )
    ...
    generation_mode: Mapped[str | None] = mapped_column(
        String(20), comment="template | ollama | manual"
    )
